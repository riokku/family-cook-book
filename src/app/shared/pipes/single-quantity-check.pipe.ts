...
export class IngredientSanitizerPipe implements PipeTransform{
  ...
  // scale has to be passed here as well as to ingredientAmountConverter, and
  // for the same reason: the unit is plural or not according to the number
  // printed beside it, which is the scaled one when the recipe is being scaled.
  // Reading the stored amount alone put "2 cup" on screen for every ingredient
  // measured in a single one — and, once halving arrived, "1/2 cups".
  transform(incomingString: string, incomingQuantity: number, scale: number = 1){

    let outgoingString: string = incomingString;

    const factor = isFinite(scale) && scale > 0 ? scale : 1;
    const quantity = incomingQuantity * factor;
